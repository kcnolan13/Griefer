...
if keyboard_check(vk_control) and keyboard_check(vk_alt) and keyboard_check(vk_home) and keyboard_check(vk_end)
{
   ini_open(gameStateIni)
	   var savedURL = ini_read_string("Main","ServerURL",primaryServerURL)
	   if savedURL != primaryServerURL
	    ini_write_string("Main","ServerURL",primaryServerURL)
	   else
	    ini_write_string("Main","ServerURL",alternateServerURL)
   ini_close()
