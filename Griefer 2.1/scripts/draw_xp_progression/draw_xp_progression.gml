/// @description draw_xp_progression(x,y,scale,halign,valign,text_color,alpha,rank,xp)
/// @function draw_xp_progression
/// @param x
/// @param y
/// @param scale
/// @param halign
...
draw_text_ext_transformed(draw_x+5*scale+global.rank_radius*scale*2,draw_y+5,string_hash_to_newline(name),0,600,scale,scale,0)
//END
...
if xp_needed > 0
{
    draw_text(left+nr_width/2,top+nr_height*1/3,string_hash_to_newline(string(floor(xp_needed))))
    draw_text(left+nr_width/2,top+nr_height*1/3+20*scale,string_hash_to_newline("XP Remaining"))
}
draw_set_font(fnt_hud)
...
if icon != spr_none
{
    ...
    draw_text_ext(nu_left+nu_width/2,top+rank_sep,string_hash_to_newline("Next Armor Set:#"+bpart_lookup(get_rank_unlock_sprite(rnum),global.BPART_NAME)),18,500)
    
    var slot_width = 72
    var slot_scale = slot_width/72
    
    draw_bpart_slot_complete_ext2(id,nu_left+nu_width/2-slot_width/2,top+nu_height*1/3,slot_scale,1*alpha)
