/// @description draw_popup_challenge(x, y, title, subtitle, description, icon, color, alpha,scale)
/// @function draw_popup_challenge
/// @param x
/// @param  y
/// @param  title
/// @param  subtitle
...
draw_bpart_slot_complete_ext2(id, left+icon_pad*scale, top+icon_pad*scale, ((height-height_less)-2*icon_pad*scale)/(72), alpha)
...
draw_set_alpha(1)
draw_set_color(c_white)
draw_text_ext_transformed(left+(net_manager.armory_ysep*3.3+net_manager.armory_sl+txt_pad+10)*scale,top+(txt_pad+8)*scale,string_hash_to_newline(dat_title), 24, width/scale-145,scale,scale,0)
draw_set_font(fnt_hud)
draw_set_valign(fa_top)
if progress_varname != "" subtitle = string(progress)+" / "+string(progress_max)+"      +"+string(xp)+" XP"
    draw_text_ext_transformed(left+(net_manager.armory_ysep*3.3+net_manager.armory_sl+txt_pad+10)*scale,top+(32+txt_pad+5)*scale,string_hash_to_newline(subtitle), 24, width/scale-145,scale,scale,0)

draw_text_ext_transformed(left+(net_manager.armory_ysep*3.3+net_manager.armory_sl+txt_pad+10)*scale,top+height/2+(txt_pad+5)*scale,string_hash_to_newline(descr), 24, width/scale-145,scale,scale,0)
draw_set_valign(fa_top)
