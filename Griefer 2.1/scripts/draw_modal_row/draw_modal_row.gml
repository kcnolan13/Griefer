/// @description draw_modal_row(x, y, color, text_color, alpha, grid_y, dgrid)
/// @function draw_modal_row
/// @param x
/// @param  y
/// @param  color
/// @param  text_color
...
else
{
    ...
    draw_text(rxdraw+5, txt_y, string_hash_to_newline(spaces+"Player"))
}
...
for (var k=kst; k < ds_grid_width(dgrid)-extra_stats; k++)
{
    ...
    draw_text(dx, txt_y, string_hash_to_newline(spaces+string(cell)))
...
if dgrid = grid
{
    ...
    draw_text(rxdraw-10,rydraw+row_height/2,string_hash_to_newline(string(grid_y+1+varRead("row_offset"))))
}
