/// @description draw_player_popup_ext(x, y, pName, rank, global_rank, trueskill, head_spr, hat_spr, alpha, is_bot)
/// @function draw_player_popup_ext
/// @param x
/// @param  y
/// @param  pName
/// @param  rank
...
draw_text_ext(sub_xoff,sub_yoff,string_hash_to_newline(rank_name), 24, width-145)
draw_set_color(c_white)
...
if not tis_bot
{
    ...
    draw_text_ext(left+width-txt_pad,sub_yoff,string_hash_to_newline(global.tskill+": "+string(trueskill)),24,width*2/3)
    ...
    draw_set_valign(fa_top)
    draw_text_ext(glob_xoff,glob_yoff,string_hash_to_newline(globranktxt), 24, width-145)
    
    if menmode() != "armory" and not net_manager.lock_armory
    {
        draw_set_alpha(0.5)
        draw_text_ext(glob_xoff,glob_yoff+28,string_hash_to_newline("[Click for Stats]"), 24, width-145)
    }
}
draw_set_alpha(1)
