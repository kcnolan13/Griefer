...
for (i=0;i<96;i+=1)
{
    if !collision_rectangle(bbox_left+i,bbox_top,bbox_right+i,bbox_bottom,block,true,true) && varRead("myX")+i > 0 && varRead("myX")+i < room_width
    {varAdd("myX",i) return true}

    if !collision_rectangle(bbox_left-i,bbox_top,bbox_right-i,bbox_bottom,block,true,true) && varRead("myX")-i > 0 && varRead("myX")-i < room_width
    {varSub("myX",i) return true}
    
    if !collision_rectangle(bbox_left,bbox_top+i,bbox_right,bbox_bottom+i,block,true,true) && varRead("myY")+i > 0 && varRead("myY")+i < room_height
    {varAdd("myY",i) return true}
    
    if !collision_rectangle(bbox_left,bbox_top-i,bbox_right,bbox_bottom-i,block,true,true) && varRead("myY")-i > 0 && varRead("myY")-i < room_height
    {varSub("myY",i) return true}
    
    if !collision_rectangle(bbox_left+i,bbox_top+i,bbox_right+i,bbox_bottom+i,block,true,true) && varRead("myX")+i > 0 && varRead("myX")+i < room_width && varRead("myY")+i > 0 && varRead("myY")+i < room_height
