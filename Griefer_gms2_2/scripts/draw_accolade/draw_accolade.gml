/// @description draw_accolade()
/// @function draw_accolade
...
if in_match()
{
    ...
    draw_text_transformed(xdraw,ydraw+yoff,string_hash_to_newline(text),txtscale,txtscale,0)
}
else
{
    ysep = 0
...
if not in_match() or match_ending()
{
    if number > 0 and hover_counter < 1
    {
        ...
        if menmode() != "lottery_steal"
            txtscale = scale
        draw_text_transformed(xdraw,ydraw+yoff+ysep,string_hash_to_newline("X "+string(number)),txtscale,txtscale,0)
    }
}
