...
//DLL INIT
GMEXPORT const double initGrieferClient(char * connURL)
{
	//attempt connection
	alive = 1;
	serverURL = connURL;
	...
	console("::: connecting to: " + string(connURL));
...
GMEXPORT const double sendCmd(char * cmd) {
	std::string s(cmd);
	string json_data = "{\"val1\": 52}";
	current_socket->emit("PING");
	current_socket->emit("JSON_test", json_data);
	return 0;
}
