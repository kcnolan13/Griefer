...
//  updater.cpp
//
//  Created by Kyle Nolan on 9/21/2015
//
...
int determinepathfilename(TCHAR * szURL, TCHAR * szpath, TCHAR * szname, TCHAR * szpathfilename)
{
	...
	if ((szname == NULL) || (szname[0] == '\0'))
		...
		_tcscpy_s(szname_copy, wcslen(szname), szname);

	if ((szpath == NULL) || (szpath[0] == '\0'))
		_tcscpy_s(szpathfilename, wcslen(szname_copy), szname_copy);
	else
	{
		_tcscpy_s(szpathfilename, wcslen(szpath), szpath);
		_tcscat_s(szpathfilename, wcslen(szname_copy), szname_copy);
	}
	return result;
}
