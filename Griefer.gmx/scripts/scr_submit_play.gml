...
{
    valid = false
    popup_alert(WVIEW/2,HVIEW/2,"Invalid Name","You must enter a player name.")
}

if valid and string_length(pname) < field_pname.min_length
{
    valid = false
    popup_alert(WVIEW/2,HVIEW/2,"Invalid Name","Your player name must be at least "+string(field_pname.min_length)+" characters")
}

if valid and string_length(pword) < field_pword.min_length
{
    valid = false
    popup_alert(WVIEW/2,HVIEW/2,"Invalid Password","Your password name must be at least "+string(field_pword.min_length)+" characters")
}

if valid and pword = field_pword.text_default
{
    valid = false
    popup_alert(WVIEW/2,HVIEW/2,"Invalid Password","You must enter a password.")
}
