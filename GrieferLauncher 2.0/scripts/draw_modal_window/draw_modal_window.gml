...
if not surface_exists(surface)
{
    if not is_3d or not surf_3d_ready
    {
        ...
        if string(text) != "" or string(header_text) != "" or (object_index = noone and string_length(string("")) > 0)
        {
            ...
            if is_loading
            {
                ...
                if show_percent
                {
                    ...
                    draw_set_halign(fa_center)
                    draw_set_valign(fa_middle)
                    draw_text(xpos,ypos,string_hash_to_newline(string(round(controller.download_percent*100))+"%"))
                    draw_set_halign(fa_left)
                    draw_set_font(font)
                }
