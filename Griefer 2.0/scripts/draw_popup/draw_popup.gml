/// @description draw_popup(x, y, title, subtitle, source, body_text_override, color, alpha)
/// @function draw_popup
/// @param x
/// @param  y
/// @param  title
...
draw_text(pleft+txt_pad,ptop+txt_pad,string_hash_to_newline(dat_title))
draw_set_font(fnt_hud)
draw_set_valign(fa_bottom)
draw_text(pleft+txt_pad,ptop+title_height-txt_pad,string_hash_to_newline(subtitle))
draw_set_valign(fa_top)
...
if string_length(body_text_override) < 1
{
    ...
    {
        draw_text(pleft+txt_pad,ptop+title_height+txt_pad+tbody_voff,string_hash_to_newline(source))
    } else if source = global.src_challenge
    {
        draw_text(pleft+txt_pad,ptop+title_height+txt_pad*2,string_hash_to_newline("Challenge Gear!"))
...
{
    draw_text_ext(pleft+txt_pad,ptop+title_height+tbody_voff+txt_pad,string_hash_to_newline(body_text_override),14,(pright - pleft))
}
