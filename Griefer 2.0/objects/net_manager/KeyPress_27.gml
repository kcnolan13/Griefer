/// @description show in-match settings
...
if not instance_exists(modal_controls) and in_match() and not match_ending()
{
    ...
    {
        xtest = floor(local_player.x+16-floor(__view_get( e__VW.WView, 0 )/2)+floor(view_xoffset*__view_get( e__VW.WView, 0 )*1/3*view_scaler))
        ytest = floor(local_player.y+16-floor(__view_get( e__VW.HView, 0 )/2)+floor(view_yoffset*__view_get( e__VW.HView, 0 )*1/3*view_scaler))
    }
    else
    {
        xtest = __view_get( e__VW.XView, 0 )
        ytest = __view_get( e__VW.YView, 0 )
    }
    
    if not global.using_gamepad or not objVarRead(local_player,"alive") or net_manager.match_countdown or net_manager.starting_match or abs(xtest-__view_get( e__VW.XView, 0 )) > 3 or (gamepad_axis_check(global.devno,gp_axisrh) or gamepad_axis_check(global.devno,gp_axisrv))
        __view_set( e__VW.XView, 0, xtest )
    if not global.using_gamepad or not objVarRead(local_player,"alive") or net_manager.match_countdown or net_manager.starting_match or abs(ytest-__view_get( e__VW.YView, 0 )) > 3 or (gamepad_axis_check(global.devno,gp_axisrh) or gamepad_axis_check(global.devno,gp_axisrv))
